...
def firstPlt(model_set, auroc, auprc, data_name, save_name, index, flag=1):
    auroc = auroc[::-1]
    auprc = auprc[::-1]
    ...
    if flag == 1:
        ...
        # AUPRC
        heatmap = ax.imshow(auprc, cmap='inferno', aspect='auto')
        ax.tick_params(axis='x', labeltop=True, labelbottom=False, )
        ax.xaxis.set_ticks_position('top')
        ax.tick_params(axis='y', length=0)
        ax.set_xticks(np.arange(len(model_set)))
        ...
        for i in range(len(data_name)):
            for j in range(len(model_set)):
                text = ax.text(j, i, '%.3f' % auprc[i, j], ha='center', va='center', color='black')
    plt.tight_layout()
...
firstPlt(model_set, eight_data_auroc, eight_data_auprc, data_name,
         save_name='Figure3A_AUROC.pdf',index='A',flag=1)
...
#          save_name='Figure3B_AUPRC.pdf',index='B',flag=0)
...
#          save_name='Figure5B_AUPRC.pdf', index='B', flag=0)
