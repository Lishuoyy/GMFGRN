...
def firstPlt(model_set, model_path, data_set, data_name, colors, save_name,index):
    ...
    model_auroc = [[] for i in range(model_num)]
    model_auprc = [[] for i in range(model_num)]

    for i, data in enumerate(data_set):
        for j in range(len(model_path)):
            if j == len(model_path) - 1:
                model_auroc[j].append(np.load(model_path[j] + data + '_result_final/' + 'AUROC_set.npy'))
                model_auprc[j].append(np.load(model_path[j] + data + '_result_final/' + 'AUPRC_set.npy'))
            else:
                model_auroc[j].append(np.load(model_path[j] + data + '_result/' + 'AUROC_set.npy'))
                model_auprc[j].append(np.load(model_path[j] + data + '_result/' + 'AUPRC_set.npy'))
    ...
    for i in range(model_num):
        boxes.append(ax.boxplot(model_auroc[i], positions=box_position[i], widths=0.2, patch_artist=True,
    ...
    legend = ax1.legend(boxes1, model_set, loc='upper left', bbox_to_anchor=(1.01, 1))
...
# data_set = ['hESC', 'mESC_2', 'mHSC_E', 'mHSC_GM', 'mHSC_L']
...
#          save_name='five_data_final_1.pdf',index='A')
...
# colors = colors[::-1]
# firstPlt(model_set, model_path, data_set, data_name, colors,
#          save_name='five_data_500_final_1.pdf',index='C')
...
colors = colors[::-1]
firstPlt(model_set, model_path, data_set, data_name, colors,
         save_name='time_data_final_1_AUROC.pdf',index='A')
