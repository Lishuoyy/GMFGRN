...
class NGCF(nn.Module):
    def __init__(self, g, in_size, layer_size, dropout, lmbd=1e-5):
        ...
        self.fc = nn.Sequential(
            nn.Linear(layer_size[-1] * 4 * 2, layer_size[-1]),
            nn.Tanh(),
            nn.Linear(layer_size[-1], 1),
        )

    def create_bpr_loss(self, users, pos_items, neg_items, pos_score, pos_rpkm_datas=None):
        ...
        pos_rpkm_datas = torch.tensor(pos_rpkm_datas, dtype=torch.float32).to(users.device)
        # print(pos_score.shape, pos_rpkm_datas.shape)
        mse_loss = nn.MSELoss(reduction="mean")
        rpkm_loss = mse_loss(pos_score.squeeze(), pos_rpkm_datas)
        ...
        return mf_loss + emb_loss + rpkm_loss, mf_loss, emb_loss, rpkm_loss
    ...
    def forward(self, g, user_key, item_key, users, pos_items, neg_items):
        ...
        pos_score = self.fc(torch.cat([u_g_embeddings, pos_i_g_embeddings], dim=1))

        return u_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings, pos_score
