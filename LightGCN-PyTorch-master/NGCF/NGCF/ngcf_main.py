...
def main(args):
    ...
    for epoch in range(args.epoch):
        t1 = time()
        loss, mf_loss, emb_loss, mse_loss = 0.0, 0.0, 0.0, 0.0
        for idx in tqdm(range(n_batch)):
            users, pos_items, neg_items, pos_rpkm_datas = data_generator.sample()
            u_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings, pos_score = model(
                g, "user", "item", users, pos_items, neg_items
            )

            batch_loss, batch_mf_loss, batch_emb_loss, batch_mse_loss = model.create_bpr_loss(
                u_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings, pos_score, pos_rpkm_datas
            )
            optimizer.zero_grad()
            batch_loss.backward()
            optimizer.step()
