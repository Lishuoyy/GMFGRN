...
class GeneData:
    def __init__(self, rpkm_path, label_path, divide_path, TF_num, gene_emb_path, cell_emb_path, istime, ish5=False,
        ...
        # x = self.origin_data
        self.gene_emb_distance = euclidean_distances(self.gene_emb)
        # self.gene_emb_distance1 = np.cov(self.origin_data)
        # np.corrcoef(self.gene_emb)
        # 对角线填充为0
        ...
        for i in range(TF_num):
            ...
            for line in self.key_list[start_idx:end_idx]:
                ...
                if int(label) != 2:
                    ...
                    # print(gene1_neighbour_cat.shape)
                    ...
                    if len(gene1_cells) == 0:
                        gene1_cells_emb = np.zeros(gene1_emb.shape[1])
                    else:
                    ...
                    if len(gene2_cells) == 0:
                        gene2_cells_emb = np.zeros(gene1_emb.shape[1])
                    else:
                    ...
                    # gene_emb = np.concatenate((gene1_emb, gene2_emb,
            ...
            print(this_datas.shape, this_cell_datas.shape)
...
def main():
    ...
    drim_path = '../modelResult/DeepDRIM/' + data_name + '_result/'
    save_dir = '../modelResult/myModel/' + data_name + '_neighbour/'
    # drim_path = '../modelResult/TimeData/dynDeepDRIM/' + data_name + '_result/'
    # save_dir = '../modelResult/TimeData/myModel/' + data_name + '_neighbour/'
    ...
    #              TF_num=TF_num, gene_emb_path='../../gcmc/timeData/' + data_name + '_emb/user_out_vfinal_1.npy',
    #              cell_emb_path='../../gcmc/timeData/' + data_name + '_emb/movie_out_vfinal_1.npy',
    #              istime=True,
    #              gene_list_path='../data_evaluation/Time_data/DB_pairs_TF_gene/' + data_name + '_gene_list_ref.txt',
    #              ish5=True, data_name=data_name)
    ...
    for fold in range(1, 4):
        ...
        model = LinearNet(emb_dim).to(device)
        ...
        test_model = LinearNet(emb_dim).to(device)
        ...
        with torch.no_grad():
            for batch_idx, (emb_data, target) in enumerate(test_loader):
                ...
                output = test_model(emb_data, None)
                output = output.squeeze()
...
def main_lightgbm():
    e = GeneData('../data_evaluation/single_cell_type/mHSC-L/ExpressionData.csv',
                 ...
                 TF_num=18, gene_emb_path='../../gcmc/mHSC_L_emb/user_out_v5.npy',
                 cell_emb_path='../../gcmc/mHSC_L_emb/movie_out_v5.npy',
                 istime=False)
    # e = GeneData('../data_evaluation/single_cell_type/mHSC-E/ExpressionData.csv',
    #              '../data_evaluation/single_cell_type/training_pairsmHSC_E.txt',
    #              '../data_evaluation/single_cell_type/training_pairsmHSC_E.txtTF_divide_pos.txt',
    ...
    # test_indel = [[25, 30, 31, 18, 15, 26, 28, 33, 3, 4, 14, 1
    for fold in range(1, 4):
        test_index = fold
        test_TF = [i for i in range(int(np.ceil((test_index - 1) * 0.333333 * TF_num)),
                                    int(np.ceil(test_index * 0.333333 * TF_num)))]
        ...
        train_emb_datas, val_emb_datas, train_h_datas, val_h_datas, train_cell_datas, val_cell_datas, \
        ...
        train_data = lgb.Dataset(train_emb_datas, label=train_labels)
        val_data = lgb.Dataset(val_emb_datas, label=val_labels)
        test_data = lgb.Dataset(test_emb_datas, label=test_labels)
        params = {
            'boosting_type': 'gbdt',
            'objective': 'binary',
        ...
        gbm = lgb.train(params, train_data, num_boost_round=1000,
                        valid_sets=[train_data, val_data], early_stopping_rounds=10)
        y_pred = gbm.predict(test_emb_datas, num_iteration=gbm.best_iteration)
        pre_all.append(y_pred)
        label_all.append(test_labels)
        test_auc = roc_auc_score(test_labels, y_pred)
        y_pred = np.where(y_pred > 0.5, 1, 0)
        test_acc = accuracy_score(test_labels, y_pred)

        print('test', test_acc, test_auc)

    pre_all = np.concatenate(pre_all, axis=0)
    label_all = np.concatenate(label_all, axis=0)
    auc_all = roc_auc_score(label_all, pre_all)
    pre_all = np.where(pre_all > 0.5, 1, 0)
    acc_all = accuracy_score(label_all, pre_all)
    print('all', acc_all, auc_all)
...
def predict_all():
    def getData(e, gene1, gene2):
        gene1 = gene1.upper()
        gene2 = gene2.upper()
        # if not e.gene_to_name:
        #     gene1_idx = e.gene_to_idx[gene1]
        #     gene2_idx = e.gene_to_idx[gene2]
        # else:
        #     gene1_index = e.gene_to_name[gene1]
        #     gene2_index = e.gene_to_name[gene2]
        #     gene1_idx = e.gene_to_idx[gene1_index]
        #     gene2_idx = e.gene_to_idx[gene2_index]
        gene1_idx = e.gene_to_idx[gene1]
        gene2_idx = e.gene_to_idx[gene2]
        gene1_emb = e.gene_emb[gene1_idx]  # + gene1_cells_emb
        gene2_emb = e.gene_emb[gene2_idx]  # + gene2_cells_emb

        gene1_emb = np.expand_dims(gene1_emb, axis=0)
        gene2_emb = np.expand_dims(gene2_emb, axis=0)

        gene1_cells = e.geneHaveCell[gene1_idx]
        if len(gene1_cells) == 0:
            gene1_cells_emb = np.zeros(gene1_emb.shape[1])
        else:
            gene1_cells_emb = e.cell_emb[gene1_cells]
            gene1_cells_emb = np.mean(gene1_cells_emb, axis=0)
        gene2_cells = e.geneHaveCell[gene2_idx]
        if len(gene2_cells) == 0:
            gene2_cells_emb = np.zeros(gene1_emb.shape[1])
        else:
            gene2_cells_emb = e.cell_emb[gene2_cells]
            gene2_cells_emb = np.mean(gene2_cells_emb, axis=0)

        gene1_cells_emb = np.expand_dims(gene1_cells_emb, axis=0)
        gene2_cells_emb = np.expand_dims(gene2_cells_emb, axis=0)
        gene_emb = np.concatenate((gene1_emb, gene2_emb, gene1_cells_emb, gene2_cells_emb), axis=0)
        gene_emb = np.expand_dims(gene_emb, axis=0)
        gene_emb = torch.from_numpy(gene_emb).float()
        return gene_emb
    ...
    e = GeneData('../data_evaluation/Time_data/scRNA_expression_data/' + data_name + '_expression_data/',
                 '../data_evaluation/Time_data/DB_pairs_TF_gene/' + data_name + '_gene_pairs_400.txt',
                 '../data_evaluation/Time_data/DB_pairs_TF_gene/' + data_name + '_gene_pairs_400_num.txt',
                 TF_num=TF_num, gene_emb_path='../../gcmc/timeData/' + data_name + '_emb/user_out_vfinal_2.npy',
                 cell_emb_path='../../gcmc/timeData/' + data_name + '_emb/movie_out_vfinal_2.npy',
                 istime=True,
                 gene_list_path='../data_evaluation/Time_data/DB_pairs_TF_gene/' + data_name + '_gene_list_ref.txt',
                 ish5=True, data_name=data_name)
    # cross_file_path = '../data_evaluation/Time_data/DB_pairs_TF_gene/' + data_name + '_cross_validation_fold_divide.txt'
    ...
    # train useing all data
    train_datas = np.concatenate(e.datas, axis=0)
    train_labels = np.concatenate(e.labels, axis=0)
    train_key_datas = np.concatenate(e.gene_key_datas, axis=0)
    train_key_dict = {}
    TFs = set()
    for i, key in enumerate(train_key_datas):
        train_key_dict[key[0] + ',' + key[1]] = int(key[2])
        TFs.add(key[0])

    print(train_datas.shape)
    print(train_labels.shape)
    print(len(train_key_dict))
    print(TFs)

    train_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(torch.from_numpy(train_datas).float(),
                                       torch.from_numpy(train_labels).float()),
        batch_size=512, shuffle=True, num_workers=14, pin_memory=True)

    model = LinearNet().to(device)
    # model.load_state_dict(
    #     torch.load('../modelResult/TimeData/myModel/' + data_name + '_result/' + '/model_weight_best.pth'))
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-6)
    criterion = nn.BCEWithLogitsLoss()
    ...
    for epoch in range(100):
        ...
        for i, (datas, labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            outputs = model(datas, None).squeeze()
